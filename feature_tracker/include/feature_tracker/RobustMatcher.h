...
public:
  RobustMatcher() : ratio_(0.8f)
  {
    ...
    detector_ = cv::makePtr<cv::OrbFeatureDetector>();
    extractor_ = cv::makePtr<cv::OrbDescriptorExtractor>();
 ...
 //Match without re-extracting features or keypoints
 void robustMatchFromPrior(std::vector<cv::DMatch>& good_matches,
