...
void RobustMatcher::robustMatchFromPrior(std::vector<cv::DMatch>& good_matches,
					 const cv::Mat& descriptors_present,
					 const cv::Mat& descriptors_past)
{
   // 2. Match the two image descriptors
  std::vector<std::vector<cv::DMatch> > matches12, matches21;

  // 2a. From image 1 to image 2
  matcher_->knnMatch(descriptors_past, descriptors_present, matches12, 2); // return 2 nearest neighbours

  // 2b. From image 2 to image 1
  matcher_->knnMatch(descriptors_present, descriptors_past, matches21, 2); // return 2 nearest neighbours
