...
/*! this function is construct function for MotionSensorDriver Class
   It will initialize all the internal variables
*/

JaguarBase::MotionSensorDriver::MotionSensorDriver()
{

  _robotConfig = new MotionConfig();
  _robotConfig->commMethod = Network;
  sprintf(_robotConfig->serialPortName, "ttyS0");

  _robotConfig->portNum = DEFAULT_PORT;
  sprintf(_robotConfig->robotID, "");
...
bool JaguarBase::MotionSensorDriver::portOpen()
{
...
void JaguarBase::MotionSensorDriver::close()
{
...
int JaguarBase::MotionSensorDriver::openSerial(const char* serialPort, const long BAUD)
{
  ...
  if (_serialfd > 0)
  {
   ...
   _stopComm = false;
   _pCommThread = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&MotionSensorDriver::commWorkingThread, this)));
     return 0;
 }
  ...
  else
  {
    ...
    switch(errno)
    {
      case EACCES:
          ...
          break;
      case ENOENT:
        extra_msg = "The requested port does not exit. Was the port name misspelled?\n";
        debug_ouput(extra_msg);
        break;
    }
   _stopComm = true;
...
void JaguarBase::MotionSensorDriver::debug_ouput(const char* errorstr)
{
#ifdef DEBUG_ERROR
    printf(" Motion Sensor: %s", errorstr);
#endif
}
int JaguarBase::MotionSensorDriver::vali_ip(const char*  ip_str)
{
...
int JaguarBase::MotionSensorDriver::openNetwork(const char* robotIP, const int portNum )
{
  char temp[512] ;
  ...
  {
    sprintf(temp, " Motion/Sensor Driver Error Message: invalid IP address: %s\n", robotIP);
    debug_ouput(temp);
    return -2;
  }
  _robotConfig->commMethod = Network;
...
void JaguarBase::MotionSensorDriver::commWorkingThread(){
  ...
  while(!_stopComm)
  {
    ...
    else if(_robotConfig->commMethod == Serial)
    {
      ...
      {
        //poll fialed -- error
        printf(" Serial Communication error, eroor no is %d: %s", errno, strerror(errno));
       _stopComm = true;
       _eCommState = Disconnected;
       ::close(_sockfd);
       _sockfd = -1;
...
unsigned char JaguarBase::MotionSensorDriver::CalculateCRC(const unsigned char *lpBuffer, const int nSize)
{
...
int JaguarBase::MotionSensorDriver::sendAck()
{
...
void JaguarBase::MotionSensorDriver::DealWithPacket(const unsigned char *lpComData, const int nLen)
{
  char debugtemp[512] ;
  int temp;
...
void JaguarBase::MotionSensorDriver::handleComData(const unsigned char *data, const int nLen)
{
...
void JaguarBase::MotionSensorDriver::debugCommMessage(std::string msg)
{
#ifdef DEBUG_ERROR
  printf(" Motion Sensor Driver: %s",msg.c_str());
#endif

}

void JaguarBase::MotionSensorDriver:: getMotionDriverConfig(MotionConfig* driverConfig)
{
...
void JaguarBase::MotionSensorDriver:: setMotionDriverConfig(MotionConfig* driverConfig)
{
...
int JaguarBase::MotionSensorDriver:: sendMotorCtrlAllCmd(CtrlMethod ctrlMethod, const int cmd1, const int cmd2, const int cmd3, const int cmd4, const int cmd5, const int cmd6, const int time)
{
...
int JaguarBase::MotionSensorDriver::sendMotorCtrlAllCmd(CtrlMethod ctrlMethod, const int cmd1, const int cmd2, const int cmd3, const int cmd4, const int cmd5, const int cmd6)
{
  unsigned char msg[255];
   short tempCmd = 0;
   msg[0] = COM_STX0; msg[1] = COM_STX1;
...
int JaguarBase::MotionSensorDriver::sendMotorCtrlCmd(CtrlMethod ctrlMethod, const int channel, const int cmd, const int time)
{
  unsigned char msg[255];
...
int JaguarBase::MotionSensorDriver::sendMotorCtrlCmd(CtrlMethod ctrlMethod,const int channel, const int cmd)
{
  unsigned char msg[255];
  if ((channel < 0) || (channel > 5))
     return -1;
...
int JaguarBase::MotionSensorDriver::sendServoCtrlAllCmd(const int cmd1, const int cmd2, const int cmd3, const int cmd4, const int cmd5, const int cmd6, const int time)
{
  unsigned char msg[255];
  short tempCmd = 0;
  short tempTime = time;
...
int JaguarBase::MotionSensorDriver::sendServoCtrlAllCmd(const int cmd1, const int cmd2, const int cmd3, const int cmd4, const int cmd5, const int cmd6)
{
  unsigned char msg[255];
  short tempCmd = 0;
  msg[0] = COM_STX0; msg[1] = COM_STX1;
...
int JaguarBase::MotionSensorDriver::sendServoCtrlCmd(const int channel, const int cmd, const int time)
{
  unsigned char msg[255];
  short tempTime = (short)( time & 0xffff);
  if ((channel < 0) || (channel > 5))
...
int JaguarBase::MotionSensorDriver::sendServoCtrlCmd(const int channel, const int cmd)
{
  unsigned char msg[255];
  if ((channel < 0) || (channel > 5))
    return -1;
...
int JaguarBase::MotionSensorDriver::disableMotorCmd(const int channel)
{
  unsigned char msg[255];
  if ((channel < 0) || (channel > 5))
    return -1;
...
int JaguarBase::MotionSensorDriver::enableMotorCmd(const int channel)
{
  unsigned char msg[255];
  if ((channel < 0) || (channel > 5))
    return -1;
...
int JaguarBase::MotionSensorDriver::disableServoCmd(const int channel)
{
  unsigned char msg[255];
  if ((channel < 0) || (channel > 5))
    return -1;
...
int JaguarBase::MotionSensorDriver::setMotorPositionCtrlPID(const int channel, const int kp, const int kd, const int ki)
{
  unsigned char msg[255];
  if ( (channel < 0) || (channel > 5) )
    return -1;
...
int JaguarBase::MotionSensorDriver::setMotorVelocityCtrlPID(const int channel, const int kp, const int kd, const int ki)
{
  unsigned char msg[255];
  if ( (channel < 0) || (channel > 5) )
  return -1;
...
int JaguarBase::MotionSensorDriver::setMotorFricCompensation(const int cmd1, const int cmd2, const int cmd3, const int cmd4, const int cmd5, const int cmd6)
{
  unsigned char msg[255];
...
int JaguarBase::MotionSensorDriver::setCustomIO(const int cmd)
{
  unsigned char msg[255];
...
int JaguarBase::MotionSensorDriver::sendPowerCtrlCmd(const int cmd)
{
  unsigned char msg[255];
...
int JaguarBase::MotionSensorDriver::sendCommand(const unsigned char* msg, const int nLen)
{
  ssize_t retval = 0;
  if (!_stopComm)
  {
